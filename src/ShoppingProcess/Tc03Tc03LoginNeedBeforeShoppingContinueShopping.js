...
async function Tc04ShoppingContinueShopping() {
    ...
    let shoppingCartConfirm = await driver.wait(until.elementLocated(By.css('#cart_title'))).getText();
     if (shoppingCartConfirm.match(/SHOPPING-CART SUMMARY/)) {
        ...
        console.log("Error Tc04ShoppingContinueShopping user is not on shopping cart summary")
     }
     ...
     if (authenticationPageConfirm.match(/AUTHENTICATION/)) {
        ...
        console.log("Error Tc04ShoppingContinueShopping user is not on authentication page")
     }
